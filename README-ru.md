...
**Кейс:** На вашем сайте доктор может пригласить пациента на видеоконференцию, не требуя от него сложных действий. В чате доктор нажимает кнопку “Пригласить в конференцию”, и пациенту появляется форма, в которой он вводит своё имя. В один клик создаётся гостевая учетная запись и конференция. Пациент автоматически подключается к конференции, не проходя дополнительных шагов.

Как это работает:

1. Доктор и пациент уже авторизованы на сайте и имеют уникальные ID.

2. Доктор на сайте нажимает кнопку для создания конференции.

3. Пациент заполняет форму, вводит своё имя (или оно автоматически берется из УЗ, если необходимо) и нажимает кнопку “Подключиться”.

4. Скрипт на сайте (или на бекенде) через API TrueConf Server автоматически создает гостевую учетную запись для пациента, используя его уникальный ID и запускает новую видеоконференцию.

5. Пациент мгновенно подключается к конференции с указанным именем и уникальным ID.
...
ALLOW_SELF_SIGNED=false              # Установите true, если используется cамоподписанный TLS-сертификатом
```
...
После этого откройте `https://localhost:<PORT>` в браузере и введите имя пользователя. Веб-приложение сформирует полезную нагрузку для конференции (включая права доступа, приглашения и другие данные) и отправит её на бэкенд. Бэкенд выполнит следующие шаги:

1. Запросит токен доступа OAuth у вашего сервера TrueConf.

2. Создаст и запустит конференцию, назначив владельцем пользователя, чей TrueConf ID был указан.
