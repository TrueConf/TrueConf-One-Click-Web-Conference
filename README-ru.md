...
> [!TIP]
> В данном примере доктор уже имеет учетную запись в TrueConf Server, поэтому мы можем использовать его TrueConf ID. В реальных кейсах, когда доктор не имеет учетную запись на сервере, ему нужно генерировать TrueConf ID на лету, как это показано ниже на примере для пациента.
...
3. Получит iframe и вернет его на фронтенд, который отобразит конференцию в формате веб-приложения на вашем сайте.
